...
double to_24_hour_clock(double hours)
{
    assert(hours >= 0);
    double d;
    double f = modf(hours, &d);
    return (int)hours % 24 + f;
    /*
...
{
    if (time - utc_offset > 0) return to_24_hour_clock(time - utc_offset);
    else return to_24_hour_clock((time - utc_offset) + 24);
...
{
    if (time + utc_offset > 0) return to_24_hour_clock(time + utc_offset);
    else return to_24_hour_clock(24 + (time + utc_offset));
    /*
...
void main()
{
    ...
    assert(fabs(time_to_utc(+3, 23.0) - 20.0) < DBL_EPSILON);  
    assert(fabs(time_to_utc(-4, 23.0) - 3.0) < DBL_EPSILON);
